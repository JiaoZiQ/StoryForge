...
pause preserves checkpoints and resume reuses the Job. Actor retry is disabled so
provider and infrastructure retries cannot multiply.
...
Application Service 仍在启动工作流前检查活跃 run；M7 额外用部分唯一索引关闭并发请求之间的竞态窗口。同章节只能存在一个 pending/running/paused run，completed/failed/cancelled 历史不受影响。该约束在 SQLite 和 PostgreSQL 均测试。

Compose 的 migrate 服务只负责 schema，API readiness 读取 `alembic_version` 并要求等于代码中的唯一 head。迁移失败时 API 不进入 ready。容器重启不改变 WorkflowRun、ChapterVersion、Evaluation 或 Fact；M11 在同一 Compose 主机上共享 SQLite checkpoint volume，但不声称跨主机高可用。

## M8 接受后索引与检索

`accept_version` 在原接受事务中把旧 accepted 版本、事实及其 memory 标为 superseded，并为新版本创建 pending index。事务提交后同步执行切分、embedding 和图抽取；成功写入 completed，失败写入脱敏 failed 状态并允许 reindex。未接受版本不会创建公开 memory。
