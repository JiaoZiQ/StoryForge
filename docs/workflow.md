...
Compose 的 migrate 服务只负责 schema，API readiness 读取 `alembic_version` 并要求等于代码中的唯一 head。迁移失败时 API 不进入 ready。容器重启不改变 WorkflowRun、ChapterVersion、Evaluation 或 Fact；SQLite checkpoint 仍是单实例边界，不声称多副本恢复。
