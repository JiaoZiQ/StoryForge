...
## M12 snapshot repetition

Global repetition candidate discovery reuses accepted MemoryChunks only when their version
is frozen in the BookSnapshot. PostgreSQL executes pgvector cosine distance; candidate,
rejected, superseded, and future chunks remain invisible. A vector match is evidence, not
an automatic defect.

## M9 调试页面

Web Retrieval 页面允许输入 query、当前章节、人物、地点、来源类型、四路开关、top-k 与字符预算。响应展示每路候选数、融合去重数、最终 score、matched sources、relation path、解释、degraded reason 和截断状态。复制按钮只复制 query/计数/来源摘要；JSON debug 不包含 embedding，内容仍经过服务端预算和前端 preview 截断。
