...
Web Evaluation 页面展示 mechanical、critic、consistency、final 与原始维度分，保留 evaluator/provider/model/prompt version 来源；它不在浏览器重算最终分。Conflict 页面只允许更新 open/resolved/ignored/false_positive 和处理备注，证据按纯文本、有限长度显示。版本页使用服务端 diff，旧 Evaluation 与 Conflict 历史不因新版本而覆盖。
