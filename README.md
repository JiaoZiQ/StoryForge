...
StoryForge 是一个面向长篇小说创作的 Python AI Agent 工程。当前实现到 Milestone 8：在既有规划、生成、评估、修订、API/CLI 与容器交付能力上，增加 PostgreSQL/pgvector 长期记忆、关系图谱和四路混合检索。
...
Compose 启动顺序是 `pgvector PostgreSQL healthy → migrate successful → api`。M8 migration 创建 `vector` extension、`vector(64)` 列和 cosine HNSW 索引。服务只绑定 `127.0.0.1`：API 为 8000，PostgreSQL 为 54329。结束时执行：
...
- 未实现 Elasticsearch、LLM reranker、前端、PDF/ePub/TTS/图片和全书级评审。
...
[ROADMAP.md](ROADMAP.md) 是交付顺序的唯一依据。Milestone 8 已实现并完成冷启动验收；本次没有开始 Milestone 9。
