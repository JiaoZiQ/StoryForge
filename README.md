...
> Milestone 10 adds provider governance, model routing, privacy policy, usage/cost
> accounting, budget enforcement, retry/rate-limit/circuit controls, and an
> offline PostgreSQL `demo-m10`. Milestone 11 has not started.
...
StoryForge 是一个面向长篇小说创作的 AI Agent 工程。当前实现到 Milestone 10：在规划、生成、评估、修订、长期记忆与 Web 控制中心之上，增加受控 Provider 路由、隐私策略、用量成本、预算和可靠性治理。
...
- Web UI 是可信本地/单用户控制面，没有认证、RBAC、多人协作、WebSocket 或异步作业队列。
...
[ROADMAP.md](ROADMAP.md) 是交付顺序的唯一依据。Milestone 10 已实现；本次没有开始 Milestone 11。
